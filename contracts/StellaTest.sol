...
contract StellaTest is ERC721A, Ownable, ReentrancyGuard {
  ...
  function whitelistMintWithOneDog(bytes32[] calldata _merkleProof) public payable {
    require(whitelistMintEnabled, "The whitelist sale is not enabled!");
    require(!whitelistClaimed[msg.sender], "Address already claimed!");
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid proof!");
   ...
   function whitelistMintWithFiveDog(bytes32[] calldata _merkleProof) public payable {
    require(whitelistMintEnabled, "The whitelist sale is not enabled!");
    require(!whitelistClaimed[msg.sender], "Address already claimed!");
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid proof!");
   ...
   function whitelistMintWithTenDog(bytes32[] calldata _merkleProof) public payable {
    require(whitelistMintEnabled, "The whitelist sale is not enabled!");
    require(!whitelistClaimed[msg.sender], "Address already claimed!");
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid proof!");
  ...
  function withdraw() public onlyOwner nonReentrant {
    ...
    (bool os, ) = payable(0xbd27A0ABAFEF1a30a180d4e2519B6A72610ef6c6).call{value: address(this).balance * 12 / 100}("");
    require(os);

    (bool hs, ) = payable(0x74777e043487e81fe63989c47f8d02E4813a2262).call{value: address(this).balance}("");
    require(hs);
  }
